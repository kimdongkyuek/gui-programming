...
//  [이 단계에서 하는 일]
//    사각형 안쪽을 노란색으로 채웁니다.
//    GDI 객체를 만들고 - 쓰고 - 되돌리고 - 지우는 4단계 규칙을 배웁니다.
//
//  [새로 나온 것]
//    HBRUSH               : 도형 안쪽을 칠하는 붓
//    CreateSolidBrush(RGB(r,g,b)) : 단색 브러시 만들기
//    SelectObject(hdc, 브러시)    : 도화지에 붓을 끼움. 반환값 = 원래 끼워져 있던 붓
//    DeleteObject(브러시)         : 다 쓴 브러시 버리기
//    
//    ★ 순서가 중요합니다 (강의노트의 "사용한 후에는 반드시 돌려 놓아야 함")
//       1) 만들고  2) SelectObject 로 끼우면서 이전 것을 oldBrush 에 보관
//       3) 그리고  4) oldBrush 를 다시 끼운 뒤 내가 만든 것을 DeleteObject
//       도화지에 끼워져 있는 상태로는 삭제가 안 되므로 반드시 되돌린 다음 지웁니다.
//       빠뜨리면 GDI 누수가 생겨 오래 쓰면 그림이 안 그려집니다.
//
//  [실행하면]
//    클릭하면 노란색으로 꽉 찬 사각형이 그려집니다.
//
//  [바뀐 곳] WM_LBUTTONDOWN
// ==========================================================================
    ...
    case WM_LBUTTONDOWN:
        {
            ...
            // 2) 도화지에 끼웁니다. 반환값(원래 끼워져 있던 브러시)은 꼭 보관!
            oldBrush = (HBRUSH)SelectObject(hdc, yellow);

            // 3) 그립니다. 안쪽이 현재 끼워진 브러시(노랑)로 칠해집니다.
            Rectangle(hdc, x - 30, y - 30, x + 30, y + 30);

            // 4) 원래 브러시로 되돌린 다음에야 내가 만든 브러시를 지울 수 있습니다.
